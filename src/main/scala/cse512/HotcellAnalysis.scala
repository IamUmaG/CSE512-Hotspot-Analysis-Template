...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  pickupInfo.createOrReplaceTempView("Df")

  val givenpointsDf = spark.sql("select x,y,z from Df where x>=" + minX + " and x<= " + maxX + " and y>= " + minY + " and y<= " + maxY + " and z>= " + minZ + " and z<= " + maxZ + " order by z,y,x").persist();
  ...
  val aggregatedValues = spark.sql("select count(*) as countval, sum(pointval) as sumval,sum(squared(pointval)) as squaredsum from Df1");
  aggregatedValues.createOrReplaceTempView("sumofPoints")

  val pointsummation = aggregatedValues.first().getLong(1);
  val squaredPointSummation = aggregatedValues.first().getDouble(2);
 // val actualpointsSum2 = aggregatedValues.first().getLong(0);

  val mean =(pointsummation.toDouble / numCells.toDouble).toDouble;
  val SD = math.sqrt(((squaredPointSummation.toDouble / numCells.toDouble) - (mean.toDouble * mean.toDouble))).toDouble
  ...
  spark.udf.register("GScore", (x: Int, y: Int, z: Int, mean:Double, sd: Double, countn: Int, sumn: Int, numcells: Int) => ((
    HotcellUtils.zscore(x, y, z, mean, sd, countn, sumn, numcells))))

  val Neighbours1 = spark.sql("select x, y, z,GScore(x,y,z,"+mean+","+SD+",ncount,sumtotal,"+numCells+") as gtstat from Df2 order by gtstat desc,x desc");
